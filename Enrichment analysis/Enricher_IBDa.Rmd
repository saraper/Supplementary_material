---
  title: "Enrichment analysis for IBDa vs Hc "
...
model.results <-readxl::read_xlsx("downloads/S1 File.xlsx")
...
signif.name <- unique(signif$external_gene_name)
...
fit <- plot(barplot(enrich.H, showCategory = 7, title = "HALLMARK gene set"))
...
signif$entrez = mapIds(org.Hs.eg.db, keys = signif$external_gene_name,  column="ENTREZID",keytype="SYMBOL",multiVals="first")
...
original_gene_list <- model.results$logFC

# name the vector
names(original_gene_list) <- model.results$ensembl_gene_id
...
df2 = model.results[model.results$ensembl_gene_id %in% dedup_ids$ENSEMBL,]
